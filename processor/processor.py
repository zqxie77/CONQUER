...
def get_loss(model, data_loader):
    logger = logging.getLogger("CONQUER.train")
    model.eval()
    device = "cuda"
...
def do_train(start_epoch, args, model, train_loader, evaluator, optimizer,
    ...
    ccl = CCL(tau=0.1, method='log') 
    ccl_weight = args.ccl_weight
    rematch_weight = args.rematch_weight
    cost_function = FeedForward_Network().cuda()
    cost_optimizer = torch.optim.Adam(cost_function.parameters(),lr=2e-6)
    ...
    for epoch in range(start_epoch, num_epoch + 1):
        ...
        for n_iter, batch in enumerate(train_loader):
            batch = {k: v.to(device) for k, v in batch.items()}
            index = batch['index']
            label_hat_batch = label_hat[index.cpu()]   
            noisy_mask = label_hat_batch == 0
            batch['label_hat'] = label_hat[index.cpu()]
 
            ret = model(batch)
            img_embeds = ret["image_embed"]   
            txt_embeds = ret["text_embed"]    
            img_noisy = img_embeds[noisy_mask]
            txt_noisy = txt_embeds[noisy_mask]
            img_noisy = torch.nn.functional.normalize(img_noisy, p=2, dim=1)
            txt_noisy = torch.nn.functional.normalize(txt_noisy, p=2, dim=1)
            
            scores = ret['scores']   # similarity matrix 
            scores_detach = scores.detach()
            B = scores_detach.size(0)
            key_point_num = random.randint(1, max(1, int(B*0.1)))
            pairs = []
            for _ in range(key_point_num):
                i = random.randint(0, B-1)
                j = random.randint(0, B-1)
                if i==j:
                    continue
                pairs.append( (i,j) )
            supervision = torch.zeros(B,B, device=scores.device)
            for (i,j) in pairs:
                supervision[i,j]=1

            cost_function.train()
            cost_output = cost_function(scores_detach)
            OT_loss = torch.sum(supervision * cost_output)
            cost_optimizer.zero_grad()
            OT_loss.backward()
            cost_optimizer.step()

            if img_noisy.shape[0] == 0:
                loss_rematch = torch.tensor(0.0, device=device)
            else:    
                # similarity: [N_noisy, N_noisy]
                sim_noisy = torch.matmul(img_noisy, txt_noisy.t())
                cost_function.eval()
                cost_noisy = cost_function(sim_noisy).detach().cpu().numpy()

                N = cost_noisy.shape[0]
                a = np.ones(N) / N  # uniform mass
                b = np.ones(N) / N

                # entropic OT
                pi = ot.sinkhorn(a, b, cost_noisy, reg=0.07)  
                pi = torch.from_numpy(pi).to(device).float()

                # soft label
                pi_v2t = pi / (pi.sum(dim=1, keepdim=True) + 1e-8)
                pi_t2v = pi / (pi.sum(dim=0, keepdim=True) + 1e-8)
                # tau = 0.07
                pred_v2t = torch.softmax(sim_noisy / 0.07, dim=1)
                pred_t2v = torch.softmax(sim_noisy.t() / 0.07, dim=1)
                
                log_pred_v2t = torch.log(pred_v2t)
                log_pred_t2v = torch.log(pred_t2v)
                log_pi_v2t = torch.log(pi_v2t + 1e-8)
                log_pi_t2v = torch.log(pi_t2v + 1e-8)
                
                kl_pi_pred_t2v = F.kl_div(log_pred_t2v, pi_t2v, reduction="batchmean")
                kl_pred_pi_t2v = F.kl_div(log_pi_t2v, pred_t2v, reduction="batchmean")
                loss_kl_t2v = kl_pi_pred_t2v + kl_pred_pi_t2v
                
                kl_pi_pred_v2t = F.kl_div(log_pred_v2t, pi_v2t, reduction="batchmean")
                kl_pred_pi_v2t = F.kl_div(log_pi_v2t, pred_v2t, reduction="batchmean")
                loss_kl_v2t = kl_pi_pred_v2t + kl_pred_pi_v2t
                loss_rematch = loss_kl_v2t + loss_kl_t2v
            
            # 只统计 bge_loss 和 tse_loss 作为 main_loss
            main_loss = ret.get('bge_loss', 0) + ret.get('tse_loss', 0)
            
            # print("bge_loss:", ret.get('bge_loss', 0), "tse_loss:", ret.get('tse_loss', 0), "main_loss:", main_loss)
            
            loss_ccl = ccl(scores)
            # print(f"main_loss: {main_loss.item()}, loss_ccl: {loss_ccl.item()}, loss_rematch: {loss_rematch.item()}")
            total_loss = main_loss + ccl_weight * loss_ccl + rematch_weight * loss_rematch 
            ...
            if (n_iter + 1) % log_period == 0:
                ...
                # 增加 main_loss、loss_ccl、loss_rematch 的详细输出
                info_str += f", main_loss: {main_loss.item():.4f}, loss_ccl: {loss_ccl.item():.4f}, loss_rematch: {loss_rematch.item():.4f}"
...
def do_inference(model, test_img_loader, test_txt_loader):
    ...
    logger = logging.getLogger("CONQUER.test")
