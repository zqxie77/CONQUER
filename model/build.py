...
import numpy as np
from .tokenselection import TexualEmbeddingLayer, VisualEmbeddingLayer
...
class RDE(nn.Module):
    ...
    def forward(self, batch):
        ...
        # return embeddings
        ret.update({"image_embed": i_feats})
        ret.update({"text_embed": t_feats})
        ...
        i_feats_norm = torch.nn.functional.normalize(i_feats, p=2, dim=-1)
        t_feats_norm = torch.nn.functional.normalize(t_feats, p=2, dim=-1)
        scores = torch.matmul(i_feats_norm, t_feats_norm.t())   # [B,B]
        ret.update({'scores': scores})
